        ...
        child: Column(
          ...
          children: [
            ...
            FadeInLeft(delay: const Duration(milliseconds: 300),
              child: const Text('Track macros, hit protein goals, and forge the best version of yourself.',
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            FadeInDown(child: const Text('Your Body Metrics',
                style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800, color: AppColors.textPrimary))),
            const SizedBox(height: 6),
            const Text('Used to calculate your BMR & daily targets',
                style: TextStyle(color: AppColors.textSecondary)),
            const SizedBox(height: 40),
            _SliderCard(label: 'Weight', value: weight, min: 30, max: 200,
                unit: 'kg', onChanged: (v) => onWeight(v)),
            const SizedBox(height: 20),
            _SliderCard(label: 'Height', value: height, min: 100, max: 250,
                unit: 'cm', onChanged: (v) => onHeight(v)),
            const SizedBox(height: 20),
            _SliderCard(label: 'Age', value: age.toDouble(), min: 10, max: 90,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _stepIndicator(3),
            const SizedBox(height: 32),
            const Text('Gender & Activity',
                style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800, color: AppColors.textPrimary)),
            const SizedBox(height: 32),
            const Text('GENDER', style: TextStyle(color: AppColors.textSecondary, fontSize: 12, letterSpacing: 1.5)),
            const SizedBox(height: 12),
            Row(children: [
              _SelectChip(label: '♂ Male',   value: 'male',   selected: gender == 'male',   onTap: onGender),
              const SizedBox(width: 12),
              _SelectChip(label: '♀ Female', value: 'female', selected: gender == 'female', onTap: onGender),
            ]),
            const SizedBox(height: 32),
            const Text('ACTIVITY LEVEL', style: TextStyle(color: AppColors.textSecondary, fontSize: 12, letterSpacing: 1.5)),
            const SizedBox(height: 12),
            ...{
              ...
              child: GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    color: activity == e.key ? AppColors.primary.withOpacity(0.15) : AppColors.card,
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(
                      color: activity == e.key ? AppColors.primary : AppColors.divider,
                      width: activity == e.key ? 2 : 1,
                    ),
                  ),
                  child: Row(children: [
                    Text(e.value.$1, style: TextStyle(
                      color: activity == e.key ? AppColors.primary : AppColors.textPrimary,
                      fontWeight: FontWeight.w600,
                    )),
                    const Spacer(),
                    Text(e.value.$2, style: const TextStyle(color: AppColors.textSecondary, fontSize: 12)),
                  ]),
                ),
              ),
            )),
...
class _GoalPage extends StatelessWidget {
  ...
  const _GoalPage({required this.goal, required this.onGoal, required this.onNext});
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('We\'ll set your calorie & macro targets accordingly',
                style: TextStyle(color: AppColors.textSecondary)),
            const SizedBox(height: 40),
            _GoalCard(
              emoji: '🔥', title: 'Lose Weight', subtitle: '500 cal deficit · High protein',
              value: 'lose', selected: goal == 'lose', onTap: onGoal,
            ),
            const SizedBox(height: 16),
            _GoalCard(
              emoji: '⚖️', title: 'Maintain Weight', subtitle: 'TDEE calories · Balanced macros',
              value: 'maintain', selected: goal == 'maintain', onTap: onGoal,
            ),
            const SizedBox(height: 16),
            _GoalCard(
              emoji: '💪', title: 'Gain Muscle', subtitle: '300 cal surplus · High protein',
              value: 'gain', selected: goal == 'gain', onTap: onGoal,
            ),
            const Spacer(),
            SizedBox(width: double.infinity,
                child: ElevatedButton(onPressed: onNext, child: const Text('Calculate My Targets →'))),
            const SizedBox(height: 20),
          ],
        ),
      ),
          ...
          children: [
            ...
            FadeInDown(delay: const Duration(milliseconds: 200),
              child: ShaderMask(
                ...
                child: const Text('Your Daily Targets',
                    style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800, color: Colors.white)),
              ),
            ),
            const SizedBox(height: 8),
            FadeInUp(delay: const Duration(milliseconds: 300),
                child: Text('BMR: ${result!['bmr']!.round()} kcal/day baseline',
...
class _ResultGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GridView.count(
      ...
      children: [
        _MacroCell(label: 'Calories', value: '${result['calories']!.round()}', unit: 'kcal', color: AppColors.calorieColor),
        _MacroCell(label: 'Protein',  value: '${result['protein']!.round()}',  unit: 'g', color: AppColors.proteinColor),
        _MacroCell(label: 'Carbs',    value: '${result['carbs']!.round()}',    unit: 'g', color: AppColors.carbsColor),
        _MacroCell(label: 'Fat',      value: '${result['fat']!.round()}',      unit: 'g', color: AppColors.fatColor),
      ],
    );
  }
}
...
class _MacroCell extends StatelessWidget {
  ...
  final Color color;
  const _MacroCell({required this.label, required this.value, required this.unit, required this.color});
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const Spacer(),
          Text(
            decimals == 0 ? '${value.round()} $unit' : '${value.toStringAsFixed(1)} $unit',
...
class _SelectChip extends StatelessWidget {
  ...
  const _SelectChip({required this.label, required this.value, required this.selected, required this.onTap});
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: Row(children: [
          ...
          Expanded(                                        // ← FIX: was unconstrained Column
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (selected) const Padding(
            padding: EdgeInsets.only(left: 8),             // ← small gap before icon
